...
func getProtoForServicePort(log logr.Logger, service *corev1.Service, portName string, portNumber int32) (string, error) {
	...
	// Either there was no app-protocols annotation, or this portName was misssing one
	switch strings.ToUpper(portName) {
	case "HTTP":
		log.Info("inferred protocol HTTP for service port based on port name",
			"port name", portName,
		...
		return "HTTP", nil
	case "HTTPS":
		log.Info("inferred protocol HTTPS for service port based on port name",
			"port name", portName,
			"port number", portNumber,
			"service", fmt.Sprintf("%s.%s", service.Name, service.Namespace),
		)
	...
	// Last resort, try to infer based on common HTTP/HTTPS ports
	switch portNumber {
	case int32(80):
		log.Info("inferred protocol HTTP for service port based on port number",
			"port name", portName,
			"port number", portNumber,
			"service", fmt.Sprintf("%s.%s", service.Name, service.Namespace),
		)
		return "HTTP", nil
	case int32(443):
		log.Info("inferred protocol HTTPS for service port based on port number",
			"port name", portName,
			"port number", portNumber,
			"service", fmt.Sprintf("%s.%s", service.Name, service.Namespace),
		)
	...
	log.Info("unable to get or infer protocol for service port, defaulting to HTTP. The \"k8s.ngrok.com/app-protocols\" annotation can be used to customize the protocol for each port. When not present, HTTP will be used for ports named HTTP or without a name on port 80, HTTPS will be used for ports named HTTPS or without a name on port 443")
	return "HTTP", nil
}
