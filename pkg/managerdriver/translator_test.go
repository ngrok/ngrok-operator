...
func TestBuildInternalAgentEndpoint(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.metadata, common.MetadataAPIString(result.Spec.Metadata), "unexpected metadata for test case: %s", tc.name)
...
func TestBuildCloudEndpoint(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			assert.Equal(t, tc.irVHost.Metadata, common.MetadataAPIString(result.Spec.Metadata), "unexpected metadata for test case")
		})
	}
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			translator := &translator{
				...
				defaultIngressMetadata: `{"note":"test-metadata"}`,
			}
	...
	for _, file := range defaultTranslatorFiles {
		...
		t.Run(filepath.Base(file), func(t *testing.T) {
			...
			for _, expectedCLEP := range tc.Expected.CloudEndpoints {
				...
				assert.Equal(t, common.MetadataAPIString(expectedCLEP.Spec.Metadata), common.MetadataAPIString(actualCLEP.Spec.Metadata))
			...
			for _, expectedAE := range tc.Expected.AgentEndpoints {
				...
				assert.Equal(t, common.MetadataAPIString(expectedAE.Spec.Metadata), common.MetadataAPIString(actualAE.Spec.Metadata))
