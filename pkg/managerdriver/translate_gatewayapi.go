...
func (t *translator) gatewayTLSTermConfigToIR(listenerTLS *gatewayv1.GatewayTLSConfig, gateway *gatewayv1.Gateway) (*ir.IRTLSTermination, error) {
	...
	// listenerTLS.Options is a map, so iteration order is nondeterministic. When a
	// user sets both the canonical and legacy form of the same suffix during
	// migration, the canonical ngrok.com/* value must win deterministically rather
	// than depending on which map entry iterates last. Track which suffixes were
	// set under the canonical prefix and skip any legacy entry for those suffixes.
	canonicalSuffixes := make(map[string]struct{})
	for key := range listenerTLS.Options {
		if suffix, ok := strings.CutPrefix(string(key), TLSOptionKeyPrefix); ok {
			canonicalSuffixes[suffix] = struct{}{}
		}
	}
	...
	for key, val := range listenerTLS.Options {
		...
		// LEGACY-PREFIX-MIGRATION: drop this skip in 1.0 (no legacy keys remain)
		if matchedPrefix == LegacyTLSOptionKeyPrefix {
			if _, ok := canonicalSuffixes[keySuffix]; ok {
				continue
			}
		}
