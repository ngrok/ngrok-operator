...
func (d *Driver) getPortAppProtocol(service *corev1.Service, port *corev1.ServicePort) (string, error) {
	...
	case "k8s.ngrok.com/http2":
		return "http2", nil
	case "":
		return "", nil
	default:
		return "", fmt.Errorf("Unsupported appProtocol: '%s', must be 'k8s.ngrok.com/http2' or ''. From: %s service: %s", proto, service.Namespace, service.Name)
	}
}
