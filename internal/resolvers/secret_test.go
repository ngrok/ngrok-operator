...
func TestDefaultSecretResolver(t *testing.T) {
	...
	value, err := defaultSecretResolver.GetSecret(context.Background(), "namespace", "name", "key")
...
func TestStaticSecretResolver(t *testing.T) {
	...
	value, err := resolver.GetSecret(context.Background(), "namespace", "name", "key")
	assert.NoError(t, err)
	assert.Equal(t, "secret-value", value)

	value, err = resolver.GetSecret(context.Background(), "non-existent-namespace", "name", "key")
	assert.EqualError(t, err, "namespace 'non-existent-namespace' not found")
	assert.Empty(t, value)

	value, err = resolver.GetSecret(context.Background(), "namespace", "non-existent-name", "key")
	assert.EqualError(t, err, "secret 'namespace/non-existent-name' not found")
	assert.Empty(t, value)

	value, err = resolver.GetSecret(context.Background(), "namespace", "name", "non-existent-key")
