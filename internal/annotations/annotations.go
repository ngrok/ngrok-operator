...
// The canonical annotation key for each user-facing or operator-written knob
// uses the new `ngrok.com/` prefix. The matching `Legacy*Annotation` const a
// few lines down is read-side compatibility for the migration window and is
// deleted in the cleanup PR — see `internal/deprecation` for the marker
// convention used to find every site.
//
...
// See docs/migration-v1-prefix.md and internal/deprecation for the convention.
const (
	LegacyComputedURLAnnotation      = "k8s.ngrok.com/computed-url"
...
// Accepts the legacy-prefix form during the migration window. Since this key
// is operator-written, legacy hits are silent (no event): the next
// setComputedURLAnnotation write re-stamps it under the new key.
func ExtractComputedURL(obj client.Object) (string, error) {
	return parser.GetStringAnnotationWithFallback(ComputedURLKey, obj, nil)
}
